...
Promise.all([
    ...
    clearcolor='rgba(189, 189, 189, 0.855)'
    ...
    d3.helping.tooltip = function(){
        ...
        function tooltip(selection){
            ...
            selection.on('mouseover', function(){
                ...
                //mouseover 하면 linegraph를 그리게한다.

                renderarea.append('path')
                          .attr('class','tooltipgraphone')
                          .datum(sdata.filter(function(d){return d.Year==yr}))
                          .attr('d',d3.line()
                          .x(function(d){return margin+xscale(d.date)})
                          .y(function(d){return height-yscale(d.textblob)}))
                          .attr('fill','none')
                ...
                renderarea.append('path')
                          .attr('class','tooltipgraphtwo')
                          .datum(sdata.filter(function(d){return d.Year==yr}))
                          .attr('d',d3.line()
                          .x(function(d){return margin+xscale(d.date)})
                          .y(function(d){return height-yscale(d.vader)}))
                          .attr('fill','none')
                          .attr("stroke-linejoin", "round")
                          .attr("stroke-linecap", "round")
                          .attr('stroke',color1)
            ...
            .on('mousemove',()=>{graphstop=true;})
            .on('mouseout', function(){
                // Remove tooltip
                setTimeout(()=>{graphstop=false},500)
                d3.selectAll('.tooltipgraphone').transition().duration(1000).style('opacity',0)
                d3.selectAll('.tooltipgraphtwo').transition().duration(1000).style('opacity',0)
        ...
        return tooltip;
        //why do I need to return tooltip???
    };
                  ...
                  .duration(1500)     
                   ...
                   .style('fill',color1)
                      ...
                      .duration(1500)     
                       ...
                       .style('fill','rgba(255, 255, 255, 0.20)')}
                    },3000)
...
function sidebar(){
    ...
    d3.select('.firstYear').append('svg').append('rect')
      .attr('x',0)
      .attr('y',0)
      .attr('width',3)
      .attr('height',1000)
    ...
    d3.select('.secondYear').append('svg').append('rect')
    .attr('x',0)
    .attr('y',0)
    .attr('width',3)
    .attr('height',1000)
    ...
    d3.select('.thirdYear').append('svg').append('rect')
    .attr('x',0)
    .attr('y',0)
    .attr('width',3)
    .attr('height',1000)
